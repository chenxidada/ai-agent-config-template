...
用户只跟一个 OpenCode 主 Session（Orchestrator）交互。Orchestrator 自动调度 9 个子 Agent 完成完整的软件工程工作流，在关键节点停下来等待人类确认，支持用户随时干预修正。
...
| 类别 | 条件 | 处理 |
|------|------|------|
| A: Pipeline 命令 | 用户用了 /feature, /bugfix, /idea, /rebuild, /fullflow | 直接走对应 pipeline |
...
4. 系统重建/大规模重写 → **rebuild**
5. 跨域大任务/不明确任务 → **fullflow**
6. 非常小的单点修改 → **short flow**
7. 以上都不匹配 → 尝试**默认倾向规则**
8. 仍无法判断 → **结构化三问澄清**
