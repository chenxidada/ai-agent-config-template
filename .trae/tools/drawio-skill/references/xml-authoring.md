...
**Edge style rules (MANDATORY — violations cause layout chaos):**
...
- **MANDATORY: Pin `exitX/exitY/entryX/entryY` on EVERY edge** — not just "when a node has 2+ connections". Unpinned edges use center-to-center auto-routing which causes crossings and overlaps in any non-trivial diagram. The only exception is edges generated by autolayout.py (Graphviz computes the routing).
...
- **MANDATORY: No two edges may share the same (exitX,exitY)→(entryX,entryY) path.** If multiple edges leave the same side of a node, they MUST use different anchor points (e.g., 0.25, 0.5, 0.75). Stacked edges are the #1 cause of "messy" diagrams.
- **MANDATORY: Before writing any edge, verify its path does not cross an unrelated node.** If it would, either: (a) choose a different exit/entry side, (b) add waypoints to route around, or (c) reposition the blocking node. Do NOT leave edge-through-shape crossings.
...
### Distributing connections on a shape (MANDATORY)

When multiple edges connect to the same shape, you MUST assign different entry/exit points to prevent stacking. **Never leave two edges using the same anchor point on the same shape — this is the primary cause of "messy lines".**
...
| Right-top | 1 | 0.25 | 2nd connection from right |
| Right-bottom | 1 | 0.75 | 3rd connection from right |
...
| Left-top | 0 | 0.25 | 2nd connection from left |
| Left-bottom | 0 | 0.75 | 3rd connection from left |

**Distribution formula:** if a shape has N connections on one side, space them evenly: for N=2 use (0.33, 0.67), for N=3 use (0.25, 0.5, 0.75), for N=4 use (0.2, 0.4, 0.6, 0.8). General formula: `anchor = (i+1) / (N+1)` for the i-th connection (0-indexed).

**Side selection strategy:**
- Edge goes UP → exit from top, enter from bottom
- Edge goes DOWN → exit from bottom, enter from top
- Edge goes RIGHT → exit from right, enter from left
- Edge goes LEFT → exit from left, enter from right
- **Diagonal connections (node not aligned horizontally or vertically):** pick the side that creates the SHORTEST path. If ambiguous, prefer the side with fewer existing connections.
...
### Layout tips (MANDATORY for hand-written XML)

**Spacing — scale with complexity (these are MINIMUMS, not suggestions):**

| Diagram complexity | Nodes | Horizontal gap | Vertical gap | Routing corridor |
|-------------------|-------|----------------|--------------|-----------------|
| Simple | ≤5 | 200px | 150px | 60px |
| Medium | 6–8 | 280px | 200px | 80px |
| Complex | >8 | 350px | 250px | 100px |

**IMPORTANT: >8 nodes should use autolayout.py instead of hand-writing. If you are hand-writing XML for >8 nodes, you are violating the MANDATORY LAYOUT RULE in SKILL.md Step 3.**

**Routing corridors (MANDATORY):** between shape rows/columns, leave an empty corridor of the size specified above where edges can route without crossing shapes. Never place a shape in a gap that edges need to traverse. The corridor width = `max(node_width * 0.5, table_minimum)`.
...
**Layout planning process (MANDATORY — do this BEFORE writing any XML):**
1. List all nodes and their connections (adjacency list)
2. Identify hub nodes (nodes with ≥3 connections) — these go in the CENTER
3. Assign nodes to layers/rows based on data flow direction (sources → top/left, sinks → bottom/right)
4. For each layer, order nodes to minimize edge crossings between layers (put connected nodes close together)
5. Calculate coordinates using the spacing table above
6. For each edge, determine which side to exit/enter from (using the side selection strategy above)
7. Verify NO edge path crosses an unrelated node — if it does, adjust node positions or add waypoints
...
**Avoiding edge-shape overlap (MANDATORY checks before finalizing):**
- Before finalizing coordinates, trace EVERY edge path mentally — if it must cross an unrelated shape, either move the shape or add waypoints. This is not optional.
...
- **Final verification:** after writing all XML, mentally walk through each edge from source to target. Count the number of unrelated shapes it crosses. If any edge crosses ≥1 unrelated shape, the layout is BROKEN and must be fixed before proceeding.
