...
- Drawio skill：`.trae/skills/drawio-skill/SKILL.md`
- Drawio 脚本：`.trae/tools/drawio-skill/scripts/`
- Drawio 参考文档：`.trae/tools/drawio-skill/references/`
...
3. 读取 `.trae/skills/drawio-skill/SKILL.md` 了解完整能力
...
| 大规模依赖图（>15 节点） | `autolayout.py` JSON → 自动布局 | `python3 .trae/tools/drawio-skill/scripts/autolayout.py graph.json -o docs/wiki/diagrams/<name>.drawio --tune` |
| C4 模型 | `c4.py` JSON → 多页面 | `python3 .trae/tools/drawio-skill/scripts/c4.py c4.json -o docs/wiki/diagrams/<name>.drawio` |
| UML 时序图 | `seqlayout.py` JSON → 精确坐标 | `python3 .trae/tools/drawio-skill/scripts/seqlayout.py seq.json -o docs/wiki/diagrams/<name>.drawio` |
| ER 图（有 SQL DDL） | `sqlerd.py` DDL → 自动布局 | `python3 .trae/tools/drawio-skill/scripts/sqlerd.py schema.sql -o docs/wiki/diagrams/<name>.drawio` |
| Python 依赖图 | `pyimports.py` | `python3 .trae/tools/drawio-skill/scripts/pyimports.py <dir> --group -o graph.json && ... autolayout` |
| 形状搜索 | `shapesearch.py` 搜索 1 万+ 官方形状 | `python3 .trae/tools/drawio-skill/scripts/shapesearch.py "<关键词>"` |

### Step 2 — 生成 .drawio 文件

- 文件名规范：`<page-slug>-<diagram-name>.drawio`（如 `architecture-system-overview.drawio`）
- 存入 `docs/wiki/diagrams/`
- 手写 XML 时先读 `.trae/tools/drawio-skill/references/xml-authoring.md`
...
```bash
python3 .trae/tools/drawio-skill/scripts/repair_png.py docs/wiki/diagrams/<name>.drawio.png
```
