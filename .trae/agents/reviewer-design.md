...
- 你的产出文件：`<spec_dir>/phases/<current_phase>/review-design.md`（`<step>` = `review-design`）
- 启动即检测：若该文件已存在（说明这是一次「重跑」——旧审查报告残留），必须在写任何新内容前先归档：
  ```bash
  PHASE_DIR=".specdev/specs/<slug>/phases/<current_phase>"
  if [ -f "$PHASE_DIR/review-design.md" ]; then
    mkdir -p "$PHASE_DIR/.archive"
    mv "$PHASE_DIR/review-design.md" "$PHASE_DIR/.archive/review-design-$(date -u +%Y%m%dT%H%M%SZ).md"
  fi
  # review-design-zh.md 同理归档
  ```
...
- **硬约束（不可违反）**：
  ...
  ③ **边界**：只归档你自己的产物（review-design.md / review-design-zh.md），不碰其他 reviewer 的 review-*.md、不碰非当前 Phase 文件、不碰 `.specdev/specs/<slug>/` 之外任何文件。
