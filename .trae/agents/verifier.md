...
5. **运行构建和 lint** — 如果构建失败：
   ...
   d. 若宿主无法运行产物（交叉编译等），编译仍是不可省略的最低底线 → 见 §无法运行时的强制降级路径（cannot-run branch）
...
10. **用严重性评估验收标准** — 映射每个标准到测试结果。仅由 implementer 的隔离测试验证的 AC 而无独立端到端验证 → 标记 PARTIAL，不是 PASS。定判决前必须回到 §无法运行时的强制降级路径（cannot-run branch）自检：PASS 是否有执行证据、是否只编译未运行（强制 PARTIAL）、残余风险是否至少 MEDIUM；若判决为 PARTIAL/FAIL，按 §主动问题上报（AC-19）列出分条问题清单。
11. **Pipeline 合规验证**：
