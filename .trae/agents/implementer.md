...
- **UI Phase 额外必读**（DAG JSON 中本 Phase `ui: true` 时）：
  ...
  - `.trae/snippets/ui-skill-usage.md` — UI 技能调用规范（了解 token 从哪来）
...
8. **构建/测试成功后更新技能**：
   - 读取并更新 `.trae/skills/project-build/SKILL.md`（如存在）
   - 读取并更新 `.trae/skills/project-test/SKILL.md`（如存在）
...
| "sub-spec 没说我不能写桩" | 说「实现 X 功能」意味着功能可工作 | 功能不工作 = 未实现，不是「以桩方式实现」。不确定就问 TRAE Agent |
...
0. **🧹 启动自清理协议（第 0 步，先于 Git 校验）**：检测 `<spec_dir>/phases/<current_phase>/implementation.md`（及 `-zh.md`）是否已存在；存在则先 `mv` 到 `.archive/implementation-$(date -u +%Y%m%dT%H%M%SZ).md` 再开始工作（详见上文 Must Do #0）。**只用 `mv`，绝不运行 git，绝不改 current-status.json，边界不出当前 Phase 目录。**
   - **例外（UI Phase 分支 B 续做）**：若 `.prototype-approved` 存在且 `implementation.md` 已含 `## Prototype` 章节 → 这是续做不是重跑，**不归档**，在原文件上追加。
