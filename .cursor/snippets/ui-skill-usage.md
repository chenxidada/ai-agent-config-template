# UI Skill 调用规范（ui-ux-pro-max · Cursor 侧）

> 本文件定义 Cursor pipeline 中各 agent 如何调用 `ui-ux-pro-max` 技能。
> 技能本体：`.cursor/skills/ui-ux-pro-max/SKILL.md`（含 67 风格 / 96 配色 / 57 字体 / 99 UX 准则 / 13 技术栈）。
>
> **背景**：本技能此前未被任何 `.cursor/agents/` 引用（`grep` 零命中），
...
**禁止**：orchestrator（Cursor Agent 调度者）自行加载本技能到自己的上下文——只由子 agent 调用。
...
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "<关键词>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "<关键词>" --stack <stack>
```
...
Playwright MCP 配置见 `.cursor/mcp.json`（项目级）与根目录 `.mcp.json`。
