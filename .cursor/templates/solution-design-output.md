...
<!--
  🔴 必填章节 —— 门禁在 hg2=passed 时会程序化校验（L1–L5），缺章节或证据不真实会 deny。

  为什么必填：架构决策若建立在推断之上，reviewer-design 会在实现完成后才拿
  「代码库既有约定」去判它 —— 偏差在最贵的地方暴露。本设计必须逐条列出它
  依赖的现状事实，每条给出可复核的证据位置。

  格式：两列表格，证据形如 `路径:行号`（反引号包裹）。
  门禁会核验：路径在仓库中真实存在（L4）、行号不超过该文件实际行数（L5）。
  → 凭印象写一个路径会被抓。每条证据都必须真正读过并定位过。

  确认度：影响架构决策的断言必须是 ✅ CONFIRMED（已读函数体 / 配置实际值）。
  ⚠️ HYPOTHESIS / ❓ UNKNOWN 不得作为设计依据 —— 先走 Stop & Explore 查实，
  或降级为「开放问题」交 HG-2 由用户决策。

  逃生舱：若本设计确实不依赖任何现有代码，必须**显式声明**（不能留空）：
  本设计不依赖现有代码：<理由>
  —— 「无依据」必须是显式声明而非沉默默认，否则无法被质疑。
-->

| 事实（本设计依赖的现状） | 证据 |
|------|------|
| <例：现有鉴权在 middleware 层完成，路由层不做校验> | `src/auth/middleware.ts:42` |

> 相关背景可引用工作流级调研报告（`repo-exploration.md`），但**证据必须是 `路径:行号`**，
> 不能只写「见调研报告」——门禁无法核验后者。
