#!/bin/bash
# ============================================
# lib/status-read.sh — 共享状态读取库 (sourced，非独立执行) · Cursor 版
# ============================================
# 供 Cursor 侧四个 hook 通过 `source` 引入的共享库。
# 本 Phase（phase-0-foundation）仅提供 read_status；
# emit_anchoring_block 由 Phase 2 追加（结构对齐 .trae/hooks/lib/status-read.sh）。
#
# 引入方（source 本文件）：
...
#   的 hook 中，read_status **绝不能裸调用**，必须用
#   `if read_status "$F"; then ... else ...; fi` 或 `read_status "$F" || { ... }` 包裹，
#   否则读取失败路径可能被 set -e 杀死整个 hook，破坏 gate 的 allow/deny 契约。
...
# ============================================
# Phase 2 Core A：输出「流程与角色锚定块」到 stdout。
# 由 session-recovery.sh（并入 JSON additionalContext 注入）与
# context-snapshot.sh（追加写入 recovery-instructions.md）共同调用，
# 确保两处锚定内容逐字一致（AC-A5），并集中维护、杜绝漂移。
#
...
- ✅ 该做：先用 Read 读取 `current-status.json` 与 `.cursor/rules/spec-workflow.mdc`，确认真实阶段后再决定委托对象。
