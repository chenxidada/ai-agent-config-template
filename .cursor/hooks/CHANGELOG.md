...
>
> · **稳定值（引用这个）**：`git diff --shortstat 6084ab9 -- . ':(exclude).cursor/hooks/CHANGELOG.md'`
> ＝ **33 文件 `+2061 / −2020`**。拆解：新增 7 文件 `+920`（`tools/test-*.sh` 共 +731、`drift-reminder.sh` +137、`emit.sh` +52）
> ／删除 4 文件 `−961`（`pipeline-advance.sh` −363、`ORCHESTRATOR_ARCHITECTURE.md` −432、`templates/validation-report.md` −92、`opencode.jsonc` −74）
> ／修改 22 文件 `+1141 / −1059`。**不含本文件自身 → 不随本文档编辑而变，可安全引用。**
> · **含本文件**：`git diff --shortstat 6084ab9` ＝ `34 文件 +2713 / −2020`，比上一行多出的 `+652` 就是本文件自己。
> 引用它必须连「截至哪个 commit」一起说，否则明天就过期 —— 旧口径正是这么错的（见 §11.6）。
> · **另一个维度**：只看已有文件（`--diff-filter=MDR`，排除新增）＝ `+1141 / −2020`；hooks 层（同样排除本文件）＝ `+937 / −966`，净减 29 行。
>
> **`+1233 / −1882`、`净减 649`、`净减 239`、`砍掉 312 行` 四处旧说法全部作废**（溯源见 §11.6）。
> 唯一稳定的定性表述：**hooks 层净减，仓库层净增；减掉的是死代码 961 行，换来的是测试（5 套件 / 133 断言）。**
>
