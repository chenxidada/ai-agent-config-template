...
>
> 本轮亲历了「数字一改就漂」：同一条命令的结果在我编辑 `AGENTS.md`/`README.md` 后
> 从 `+2780` 一路变到 `+2792`。**故权威形式是「命令 + 截至的 commit」，不是数字本身。**
>
> · **已提交基线（引用这个）**：`git diff --shortstat 6084ab9 HEAD -- . ':(exclude).cursor/hooks/CHANGELOG.md'`
> ＝ **33 文件 `+2061 / −2020`**。按状态拆解（同口径实测，勿手算）：
> 新增 7 文件 `+920`（`tools/test-*.sh`、`drift-reminder.sh` +137、`emit.sh` +52）
> ／删除 4 文件 `−961`（`pipeline-advance.sh` −363、`ORCHESTRATOR_ARCHITECTURE.md` −432、
> `templates/validation-report.md` −92、`opencode.jsonc` −74）／修改 22 文件 `+1141 / −1059`。
> **这是 `6084ab9..HEAD` 的提交间差异，不含未提交改动 → 不随工作区编辑而变，可安全引用。**
> · **本轮 v8.4（未提交）**：`git diff --shortstat HEAD -- . ':(exclude).cursor/hooks/CHANGELOG.md'`
> ＝ **14 文件 `+741 / −77`**（相对 `16887e0`；无新增/删除，全部是修改）。
> · **含本文件**：把基线命令末尾的 `-- . ':(exclude)...'` 去掉 → 约 `+877` 更多
> （就是本文件自己；该值每次编辑本文件都会变，故只给相对量）。
> · **另一个维度**：hooks 层（截至工作区，排除本文件）＝ 8 文件 `+1048 / −966`，**净增 82 行**
